...
# Try torch_scatter; fall back to a slower implementation if missing
try:
...
class SimplicialMessagePassing(nn.Module):
    ...
    Three steps:
    1. Step1: Pairwise stream — aggregate 1-hop neighbors with attention.
    ...
    Args:
        ...
        triangle_dim (int): Triangle feature dimension; default None (same as in_dim).
        ...
        batch_size_triangles (int, optional): Node batch size for triangle updates (GPU memory).
            None disables batching. Typical values: 1000–5000.
    """
    ...
    def __init__(self, in_dim, out_dim, triangle_dim=None, lambda_high=1.0, dropout=0.0,
        ...
        # Learnable gate: blend pairwise vs higher-order (w_pair * m_pairwise + w_high * lambda * m_high_order)
    ...
    def reset_parameters(self):
        ...
        if hasattr(self.gate_linear, 'weight'):
            ...
            if self.gate_linear.bias is not None:
                # Bias init so gate starts near [0.5, 0.5]
    ...
    def forward(self, h, edge_index, triangles_dict, h_triangles):
        ...
        Args:
            h (torch.Tensor): Node features [num_nodes, in_dim].
            edge_index (torch.Tensor): Edges [num_edges, 2].
            triangles_dict (dict): node_id -> list of triangle indices containing that node.
            h_triangles (torch.Tensor): Triangle features [num_triangles, in_dim].
        ...
        # Step1: Pairwise stream — vectorized 1-hop aggregation
        h_transformed = self.W1(h)  # [num_nodes, out_dim]

        row, col = edge_index[:, 0], edge_index[:, 1]  # [num_edges]

        # Assume edge_index already has both directions for undirected graphs
        row_all, col_all = row, col

        h_v_all = h_transformed[row_all]  # target (row) [num_edges, out_dim]
        h_u_all = h_transformed[col_all]  # source / neighbor [num_edges, out_dim]

        concat_feat_all = torch.cat([h_v_all, h_u_all], dim=1)  # [num_edges, 2*out_dim]

        attention_scores_all = torch.matmul(concat_feat_all, self.a1)
        attention_scores_all = F.leaky_relu(attention_scores_all, negative_slope=self.negative_slope)
        attention_scores_all = attention_scores_all.squeeze(1)

        if HAS_TORCH_SCATTER:
            attention_weights_all = scatter_softmax(attention_scores_all, row_all, dim=0)
            weighted_features = attention_weights_all.unsqueeze(1) * h_u_all
            m_pairwise = scatter_add(weighted_features, row_all, dim=0, dim_size=num_nodes)
        else:
            ...
            for v in unique_nodes:
                ...
                if not mask.any():
                    continue
                scores_v = attention_scores_all[mask]
                neighbors_v = h_u_all[mask]
                weights_v = F.softmax(scores_v, dim=0)
                m_pairwise[v] = torch.sum(weights_v.unsqueeze(1) * neighbors_v, dim=0)

        # Step2: Higher-order stream — triangles to nodes
        if self.batch_size_triangles is None or self.batch_size_triangles >= num_nodes:
            if h_triangles.device.type == 'cpu':
                ...
                h_triangles_transformed = self.W2(h_triangles_gpu)
                del h_triangles_gpu
            else:
                h_triangles_transformed = self.W2(h_triangles)
            ...
            if len(node_indices) == 0:
                ...
                node_indices_tensor = torch.tensor(node_indices, dtype=torch.long, device=device)
                triangle_indices_tensor = torch.tensor(triangle_indices_list, dtype=torch.long, device=device)

                h_v_all = h_transformed[node_indices_tensor]
                h_T_all = h_triangles_transformed[triangle_indices_tensor]

                concat_feat_all = torch.cat([h_v_all, h_T_all], dim=1)

                attention_scores_all = torch.matmul(concat_feat_all, self.a2)
                attention_scores_all = F.leaky_relu(attention_scores_all, negative_slope=self.negative_slope)
                attention_scores_all = attention_scores_all.squeeze(1)

                if HAS_TORCH_SCATTER:
                    attention_weights_all = scatter_softmax(attention_scores_all, node_indices_tensor, dim=0)
                    weighted_features = attention_weights_all.unsqueeze(1) * h_T_all
                    m_high_order = scatter_add(weighted_features, node_indices_tensor, dim=0, dim_size=num_nodes)
                else:
                    m_high_order = torch.zeros(num_nodes, self.out_dim, device=device)
            ...
            if len(nodes_with_triangles) == 0:
                ...
                for batch_idx in range(num_batches):
                    ...
                    if h_triangles.device.type == 'cpu':
                        h_triangles_batch = h_triangles[unique_triangle_indices.cpu()].to(device)
                    else:
                        h_triangles_batch = h_triangles[unique_triangle_indices]
                    h_triangles_transformed_batch = self.W2(h_triangles_batch)
                    ...
                    h_v_batch = h_transformed[batch_node_tensor]
                    h_T_batch = h_triangles_transformed_batch[batch_triangle_indices_mapped]

                    concat_feat_batch = torch.cat([h_v_batch, h_T_batch], dim=1)

                    attention_scores_batch = torch.matmul(concat_feat_batch, self.a2)
                    attention_scores_batch = F.leaky_relu(attention_scores_batch, negative_slope=self.negative_slope)
                    attention_scores_batch = attention_scores_batch.squeeze(1)
        ...
        h_self = self.W0(h)
        gate_in = torch.cat([h_self, m_pairwise, m_high_order], dim=1)
        gate_weights = F.softmax(self.gate_linear(gate_in), dim=1)
        w_pair = gate_weights[:, 0:1]
        w_high = gate_weights[:, 1:2]
...
class SimplicialGNN(nn.Module):
    ...
    Args:
        ...
        hidden_dim (int): Hidden size; default 64.
        ...
        negative_slope (float): LeakyReLU negative slope; default 0.2.
        batch_size_triangles (int, optional): Node batch size for triangles (GPU memory).
            None disables batching. Typical: 1000–5000.
    """
    ...
    def __init__(self, input_dim, hidden_dim=64, output_dim=12, num_layers=2,
        ...
        # h_triangles should match input_dim; W2 maps triangles to hidden_dim inside each layer
    ...
    def forward(self, h_0, edge_index, triangles_dict, h_triangles):
        ...
        Args:
            ...
            edge_index (torch.Tensor): Edges [num_edges, 2].
            triangles_dict (dict): node -> triangle index list.
...
def build_node_to_triangles_dict(triangles, num_nodes):
    ...
    Returns:
        dict: node_id -> list of triangle indices incident to that node.
    """
...
def compute_loss(logits, labels, train_idx, weight_decay=0.0, model=None):
    """
    Cross-entropy plus optional L2 regularization.

    Args:
        logits (torch.Tensor): Predictions [num_nodes, num_classes].
        labels (torch.Tensor): Labels [num_nodes].
...
def evaluate(model, h_0, edge_index, triangles_dict, h_triangles, labels, idx):
    """
    Evaluate accuracy on a node subset.
